...
export const ABOUT = {
  ...
  introductions: [
    ...
    },
    {
      ...
      date: "2020年2月",
      content: `データセンターのオペレーション業務やサーバー監視業務のほか、業務改善提案や実施に取り組みました。
      入館カードに2日分の入館者情報を読み込ませる作業は、サイトから出力したCSVファイルをエクセルのマクロで取り込み、人の手でセルフチェックをしていました。
      スクレイピングとデータ取込、出力、チェックをマクロで行い、最初の1枚と最後の1枚をチェックすることで作業が完了させるように改修した経験があります。（4時間の作業を30分に短縮）
    ...
    },
    {
      ...
      subtitle:"実務経験を積むために",
      date: "2021年9月",
      content: `SES企業に転職し、業務/学習管理・金融・ECサイトなど様々な実務経験を積み、コーディングや設計書等の作成能力以外に、顧客折衝やメンバーマネジメントも経験しました。
      
      当時は案件を選べない体制のため、短期間のアサインを繰り返したことで広く浅い習得になりました。そのため、アサイン中は休日に簡単なアプリ等を作ることで業務に必要な知識や技術のインプット/アウトプットをしていました。
      メンバーリーダーやPLとしてアサインすることもあり、レビューやチケット駆動開発でタスクの振り分けなどを担当したこともあります。`,
      tagName: "HTML/CSS,JavaScript,Node.js,Vue.js,SpringBoot,Python,Go,PHP,C#,WordPress,Lambda,S3,EC2,DynamoDB,Git,Subversion,Terraform"
    },
    {
      idx: "4",
      subtitle:"現在",
      date: "2023年4月",
      content: `コンサルティングファーム企業のWeb開発や業務システム開発を中心に基本設計〜運用保守を担当しています。
      JavaやPHPを使用した既存システムのリバースエンジニアリングやリファクタリング、リプレイス/リライト、DBチューニング、バッチ最適化など、より良いシステムにするための業務に携わっています。
      
      副業では、Next.jsとSolidityを使用したブロックチェーン開発や、Next.js、Laravelを使用したWeb開発に携わっています。`,
      tagName: "HTML/CSS,JavaScript,Node.js,Next.js,Java,Solidity,PHP,Laravel,MySQL,PostgreSQL,Git,Subversion,Terraform"
    },
  ],
  arrows: [
    {fontName: "chevronRight"},
    {fontName: "chevronLeft"},
  ],
} as const;
